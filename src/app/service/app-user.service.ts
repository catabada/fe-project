...
import {AppUser, appUsers, userInfos} from "../model/user.model";
...
export class AppUserService {
  ...
  public getUserInfo(id: number): Observable<AppServiceResult<UserInfo>> {
    return new Observable<AppServiceResult<UserInfo>>(observer => {
      let userInfo = userInfos.find(userInfo => userInfo.userId === id)!
      observer.next(new AppServiceResult<UserInfo>(true, 0, "Success", userInfo))
    })
  }
